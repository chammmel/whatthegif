...
use crate::generated::communication::RoomInfoResponse;
...
async fn data_event_handler(
  ...
  match data_result {
    DataResult::JoinRequest(data) => {
      join::join_request(data, &device_id, &store, &origin).await
    }
    DataResult::PreJoinRequest(data) => {
      join::pre_join_request(data, &device_id, &store, &origin).await
    }
    DataResult::RoomInfoRequest(data) => {
      room::room_info_request(data, &store, &origin).await
    }
    DataResult::CreateRoomRequest(data) =>
      room::create_room_request(data, &store, &origin).await
    }
}
